 ...
 */
function shouldProcessCharge(charge: PaymentCharge): boolean {
  const {
...
export async function processStripeEvent(
  ...
  // FAIL-CLOSED: constructWebhookEvent always verifies the signature and
  // throws when it's missing or invalid. (The legacy code fell back to
  // trusting JSON.parse(body) when no webhook secret was configured, which
  // let a forged charge.succeeded mark arbitrary orders as paid.)
  const event = deps.payments.constructWebhookEvent(body, webhookSignature);
  ...
  if (category === 'charge') {
    const charge: PaymentCharge = event.data.object;
