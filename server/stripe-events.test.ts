...
describe('charge.created', () => {
  ...
  beforeEach(() => {
    ...
    payments = {
      // The unit tests exercise event processing, not signature verification
      // (which has its own tests against the payments drivers).
      constructWebhookEvent: (body: string) => JSON.parse(body),
    } as any;
