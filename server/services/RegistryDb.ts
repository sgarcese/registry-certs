...
export default class RegistryDb {
  ...
  async addOrder(
    ...
    tracking: boolean,
    {
      ...
      contactName,
      contactEmail,
      // confirmContactEmail is in AddOrderOptions but intentionally never
      // persisted (and never logged: it is PII).
      contactPhone,
      shippingName,
      shippingCompany,
      shippingAddr1,
    ...
    // confirmContactEmail is collected and validated but intentionally never
    // persisted. (Do not log it: it is PII.)
  ...
  async addMarriageIntentionCertificateRequest({
    ...
    const B_ZipCode =
      BResidenceCountry && BResidenceCountry !== 'USA'
        ? BResidenceCountry.toLocaleUpperCase()
        : BZIPCode;
    ...
    if (!recordset || recordset.length === 0) {
      // Failing loudly here matters: the legacy code returned a fake success
      // key (12345) on an empty recordset, silently losing the intention.
      throw new Error(
        'Recordset for creating a marriage intention came back empty'
      );
    }
...
export async function makeFixtureRegistryDbFactory(
  fixtureName: string
): Promise<Pick<RegistryDbFactory, 'registryDb' | 'cleanup'>> {
