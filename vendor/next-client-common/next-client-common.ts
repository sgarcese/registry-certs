...
/**
 * Server-side rendering used to call back into the app through hapi's
 * #inject. The standalone app instead does a real HTTP request against the
 * local server (Node's built-in fetch).
 */
function localServerUrl(path: string): string {
...
async function serverFetchGraphql<T>(
  ...
  if (parentRequest && parentRequest.headers.cookie) {
  ...
  const out = handleGraphqlResponse<T>(res.status === 200, json);
  if (cache && cacheKey) {
    cache[cacheKey] = out;
  }
  return out;
...
async function serverFetchJson<T>(
  ...
  const headers = {
    ...(parentRequest.headers.cookie
      ? { cookie: parentRequest.headers.cookie }
      : {}),
    ...(init.headers || {}),
  } as { [key: string]: string };

  const res = await fetch(localServerUrl(path), {
